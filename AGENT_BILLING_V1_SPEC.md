...
- **O3. Integración ePayco con SDK oficial + wrapper** (`src/lib/billing/epayco-client.ts`). Wrapper sobre el SDK oficial `epayco-sdk-node` v1.4.4 (npm). El wrapper expone la misma API pública que la v1 (custom fetch) para que `server.ts` y `epayco-webhook.ts` no cambien. **Decisión D1 corregida 2026-06-25**: usar SDK oficial en vez de custom fetch. Razones en §11. Config: `EPAYCO_PUBLIC_KEY`, `EPAYCO_PRIVATE_KEY`, `EPAYCO_TEST_MODE` en `.env`.
...
- **D1. ✅ CERRADA 2026-06-25 (refactor v2)**: SDK oficial `epayco-sdk-node` v1.4.4. Originalmente era fetch directo; el founder encontró la doc oficial actualizada (Smart Checkout v2) que muestra que el SDK es el path oficial. Swappeo a SDK con wrapper que mantiene la API pública. **Lección CTO (memoria personal L5)**: cuando se toca una API de vendor, leer la doc oficial PRIMERO y verificar la fecha "Updated on" antes de elegir SDK vs custom. **Anti-patrón que ya ocurrió**: elegir custom fetch basado en research viejo y perder 1 día de refactor.
