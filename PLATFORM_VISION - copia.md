...
- Soporta fuentes internas y externas (ver §3).
...
- **Archivos adjuntos al hilo:** PDFs, DOCX, XLSX subidos en el chat.
- **Bóvedas del usuario:** colecciones persistentes de documentos (ver §8).
- **Base de datos propietaria de la plataforma:** consultas SQL estructuradas por el agente.
- **Mecanismo:** Arquitectura Híbrida Integrada (§3.2).
...
- **Web search:** DuckDuckGo (✅ implementado) o API de búsqueda.
- **Scraping curado:** fuentes públicas predefinidas (boletines oficiales, gacetas, diarios jurídicos).
- **Navegación interactiva:** modo Computador (§6) para investigación profunda.
...
**Las carpetas reemplazan completamente el concepto actual de "Espacios".** Migración: la tabla `spaces` evoluciona a `folders` con `parent_id` para jerarquía.
```
📁 Clientes
  📁 Cliente A
    📁 Proyecto 1 (hilos, archivos, skills específicas)
    📁 Proyecto 2
  📁 Cliente B
    📁 Litigio X
    📁 Consultoría Y
...
| Modo Computador | ❌ Parcial | Mini-apps + browser persistente | — |
